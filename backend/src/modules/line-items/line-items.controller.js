...
export async function getLineItemsByPriority(req, res, next) {
  try {
    const {
      priority,
      page = 1,
    ...
    if (category && category.trim() !== "" && category !== "ALL") {
      conditions.push(
        `(poli.category = $${paramNum++} OR (poli.category IS NULL AND poli.region = $${paramNum++}))`,
      );
    ...
    // Apply style filter
    if (style && style.trim() !== "" && style !== "ALL") {
      conditions.push(`poli.style = $${paramNum++}`);
      params.push(style);
    }
    ...
    // Get paginated data with exact same structure as PO Details API
    const dataQuery = `
      SELECT 
        poli.id,
        poli.po_id,
        ...
        poli.quantity,
        poli.received_qty,
        (poli.quantity - COALESCE(poli.received_qty, 0)) as pending_qty,
        poli.price,
        poli.expected_delivery_date,
        poli.status,
        poli.line_priority,
    ...
    // Format response to match PO Details API exactly
    const formattedLineItems = lineItems.map((item) => ({
      ...item,
      quantity: item.quantity?.toString() || "0",
      received_qty: item.received_qty?.toString() || "0",
      pending_qty: item.pending_qty?.toString() || "0",
      price: item.price?.toString() || "0",
      weight: item.weight?.toString() || "0",
      design_code: item.design_code?.toString() || "0",
      combination_code: item.combination_code?.toString() || "0",
      product_code: item.product_code?.toString() || "0",
    }));
    ...
    console.error("Error in getLineItemsByPriority:", error);
    next(error);
  }
}
