...
export async function login(email, password) {
  ...
  try {
    ...
    if (!isPasswordValid) {
      logger.warn(
        `[${requestId}] ⚠️  AUTHENTICATION FAILED - Invalid Password`,
        {
          email,
          userId: users.id,
    ...
    if (!users.is_active) {
      logger.warn(
        `[${requestId}] ⚠️  AUTHORIZATION FAILED - Account Inactive`,
        {
          userId: users.id,
          email: users.email,
    ...
    if (users.role === "VENDOR" && users.vendor_id) {
      ...
      if (!vendor) {
        ...
        throw new UnauthorizedError(
          "Your vendor account information is incomplete.",
        );
      }
      ...
      if (vendor.status !== "ACTIVE") {
        logger.warn(
          `[${requestId}] ⚠️  AUTHORIZATION FAILED - Vendor Not Approved`,
          {
            userId: users.id,
            vendorId: users.vendor_id,
            email: users.email,
      ...
      logger.debug(
        `[${requestId}] ✅ Vendor Approval Status Verified - ACTIVE`,
        {
          userId: users.id,
          vendorId: users.vendor_id,
          vendorName: vendor.name,
    ...
    // Use role-based session timeout
    const expiresIn =
      config.jwt.sessionTimeouts[users.role] || config.jwt.expiresIn;

    logger.debug(`[${requestId}] ⏰ Setting Token Expiration`, {
      userId: users.id,
      role: users.role,
      expiresIn,
    });
    ...
    if (error instanceof UnauthorizedError) {
      ...
      logger.warn(
        `[${requestId}] 🎯 AUTHENTICATION SERVICE EVENT - FAILURE`,
        error,
        {
